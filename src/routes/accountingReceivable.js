...
/**
 * GET /api/accounting/receivable
 * Lista todas las cuentas por cobrar
 */
router.get('/receivable', authMiddleware, async (req, res) => {
  try {
...
/**
 * GET /api/accounting/receivable/:id
 * Obtiene detalle de una cuenta por cobrar específica
 */
router.get('/receivable/:id', authMiddleware, async (req, res) => {
  try {
    const schema = await getSchemaName(req);
    if (!schema) return res.status(400).json({ error: 'Business context required' });
...
/**
 * POST /api/accounting/receivable/:id/register-payment
 * Registra un pago para una cuenta por cobrar
 */
router.post('/receivable/:id/register-payment', authMiddleware, async (req, res) => {
...
// ─────────────────────────────────────────────────────────────
// ALIAS /receivables (plural) — usados por AccountingReceivablePage
// ─────────────────────────────────────────────────────────────
...
router.get('/receivables', authMiddleware, async (req, res) => {
  try {
    const schema = await getSchemaName(req);
    ...
    res.json({ success: true, data: result.rows, summary: summary.rows[0], pagination: { page: parseInt(page), limit: parseInt(limit), total, totalPages: Math.ceil(total / parseInt(limit)) } });
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, error: err.message });
  }
...
router.get('/receivables/export', authMiddleware, async (req, res) => {
  try {
    const schema = await getSchemaName(req);
    await ensureReceivablesTable(schema);
    const result = await query(`
...
router.post('/receivables', authMiddleware, async (req, res) => {
  try {
    const schema = await getSchemaName(req);
    await ensureReceivablesTable(schema);
...
// PUT /api/accounting/receivables/:id
router.put('/receivables/:id', authMiddleware, async (req, res) => {
  try {
    const schema = await getSchemaName(req);
    await ensureReceivablesTable(schema);
    const { id } = req.params;
...
// DELETE /api/accounting/receivables/:id
router.delete('/receivables/:id', authMiddleware, async (req, res) => {
  try {
    const schema = await getSchemaName(req);
    await ensureReceivablesTable(schema);
    const { id } = req.params;
...
// POST /api/accounting/receivables/:id/payments
router.post('/receivables/:id/payments', authMiddleware, async (req, res) => {
  const client = await getClient();
  try {
    const schema = await getSchemaName(req);
    await ensureReceivablesTable(schema);
