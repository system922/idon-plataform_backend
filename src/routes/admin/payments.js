...
router.post('/email/send-template', async (req, res, next) => {
  try {
    ...
    const wrap = (title, subtitle, body) => `
      <div style="font-family:Arial,sans-serif;max-width:520px;margin:auto;color:#1e293b">
        <div style="background:#ff8c42;padding:24px 28px;border-radius:10px 10px 0 0">
          <h2 style="color:#fff;margin:0;font-size:20px">IDON</h2>
        ...
        <div style="background:#f8fafc;padding:24px 28px;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 10px 10px">
          ...
          <p style="margin:20px 0 0;font-size:11px;color:#cbd5e1;text-align:center">
            IDON — mensaje automático, no responder a este correo.
          </p>
        </div>
      </div>`;
    ...
    const tpls = {
      bienvenida: {
        subject: `Bienvenido a IDON — ${businessName}`,
        html: wrap('Bienvenida', 'Tu cuenta está lista', `
          <p style="margin:0 0 8px">Estimado/a <strong>${ownerName || 'usuario'}</strong>,</p>
          <p style="margin:0 0 20px;color:#475569;font-size:14px">
            Tu negocio <strong>${businessName}</strong> ya está activo en <strong>IDON</strong>. Desde ahora puedes gestionar tus ventas, inventario, empleados y más desde un solo lugar.
          </p>
      ...
      recordatorio_pago: {
        subject: `Recordatorio de pago IDON — ${businessName}`,
      ...
      suspension: {
        subject: `Suscripción suspendida IDON — ${businessName}`,
      ...
      activacion: {
        subject: `Suscripción activada IDON — ${businessName}`,
    ...
    await sendGenericEmail({ to, subject: tpl.subject, html: tpl.html, businessName: 'IDON PLATAFORM' });
