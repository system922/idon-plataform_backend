...
router.post('/generate', authMiddleware, async (req, res) => {
  try {
    ...
    for (const emp of employees) {
      // 🔥 OBTENER ASISTENCIAS (para ambos casos, para mostrar horas)
      ...
      if (payment_type === 'daily') {
        // 🔥 PAGO DIARIO: Paga el sueldo fijo de la BD, pero muestra las horas trabajadas
        const total_pay = salary; // Sueldo fijo, sin importar horas
        
        console.log(`💰 ${emp.full_name}: Pago Diario Fijo = ${total_pay} (Horas: ${total_hours.toFixed(2)}, Días: ${daysWorked})`);
        ...
        const hourly_rate = salary / 240; // Asumiendo 30 días * 8 horas
        const normal_pay = total_hours * hourly_rate;
        const extra_pay = extra_hours * hourly_rate * 1.5;
        const total_pay = normal_pay + extra_pay;
        
        console.log(`💰 ${emp.full_name}: Pago por Horas = ${total_pay.toFixed(2)} (Horas: ${total_hours.toFixed(2)}, Extras: ${extra_hours.toFixed(2)})`);
