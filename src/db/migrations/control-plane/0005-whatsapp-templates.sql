-- Migration: 0005-whatsapp_templates.sql
-- ============================================================
-- TABLA: whatsapp_templates
-- ============================================================
-- Plantillas de mensajes WhatsApp para notificaciones automáticas
-- de la plataforma IDON. Cada plantilla tiene un tipo único
-- que identifica el propósito del mensaje.
--
-- Tipos disponibles:
-- - 'registro': Bienvenida al registrar un negocio
-- - 'aprobacion': Notificación de aprobación de negocio
-- - 'suscripcion': Activación de suscripción
-- - 'recordatorio': Recordatorio de pago
-- - 'pago_recibido': Confirmación de pago recibido
--
-- Columnas:
-- - type: VARCHAR(50) PRIMARY KEY — identificador único de la plantilla
-- - label: nombre descriptivo (ej: 'Bienvenida de Registro')
-- - description: descripción del propósito
-- - body: contenido del mensaje con variables {{variable}}
-- - variables: array de nombres de variables que usa el body
--   (ej: ['firstName', 'lastName', 'businessName'])
-- - is_active: si la plantilla está disponible para usar
-- - updated_at: última fecha de modificación
--
-- Relaciones:
-- - Usada por el servicio de notificaciones WhatsApp
-- - Las variables se reemplazan con datos del negocio/usuario
--
-- Nota: El body usa sintaxis {{variable}} para sustitución dinámica.
